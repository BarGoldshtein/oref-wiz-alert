...
def sd_ready():    _sd_notify("READY=1")
def sd_watchdog(): _sd_notify("WATCHDOG=1")
...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler(LOG_FILE), logging.StreamHandler(), BufferHandler()],
)
...
# ─── Alert categories — default colors ───────────────────────────────────────
ALERT_DEFAULTS = {
...
# Flash patterns — (on|off flag, duration_ms)
# "on" uses the alert color, "off" uses black
FLASH_PATTERNS = {
...
DEFAULT_PATTERN_MAP = {
    "1": "fast_strobe", "2": "fast_strobe", "3": "medium_strobe",
    "4": "medium_strobe", "5": "slow_pulse", "6": "slow_pulse",
    "7": "slow_pulse", "8": "slow_pulse", "13": "fast_strobe", "101": "solid",
}

# ─── BLE ──────────────────────────────────────────────────────────────────────
BLE_CHAR_UUID = "0000fff3-0000-1000-8000-00805f9b34fb"

def ble_color_cmd(r: int, g: int, b: int) -> bytes:
...
def apply_brightness(r: int, g: int, b: int, pct: int) -> tuple:
    """Scale RGB by brightness percentage (1-100)."""
    f = max(1, min(100, pct)) / 100
    return int(r * f), int(g * f), int(b * f)

# Default idle: dim warm white at 15%
...
DEFAULT_CONFIG = {
    ...
    # Per-category alert color overrides (merged with ALERT_DEFAULTS)
    "alert_colors": {},
}
...
def get_alert_color(cfg: dict, cat: str) -> dict:
    """Merge default alert color with any user overrides."""
...
class BLEManager:
    ...
    async def maintain_loop(self):
        """Keep all BLE connections alive forever."""
        while True:
            cfg = load_config()
            ...
            async with self._lock:
                ...
                for addr in [a for a in self.clients if a not in addresses]:
                    try: await self.clients[addr].disconnect()
                    except: pass
                    del self.clients[addr]
                    log.info(f"BLE removed: {addr}")
                # Connect missing
                for addr in addresses:
                    ...
                    if client is None or not client.is_connected:
                        try:
                            ...
                            await c.connect()
                            self.clients[addr] = c
                            log.info(f"BLE connected: {addr}")
                            # Apply idle state on fresh connection
                            if not self._alert_active:
                                await self._apply_idle_single(c, load_config())
                        except Exception as e:
                            log.warning(f"BLE connect failed {addr}: {e}")
            await asyncio.sleep(15)

    async def _write_all(self, cmd: bytes):
        """Send command to all connected bulbs simultaneously."""
        async with self._lock:
            tasks = [
            ...
            if tasks:
                ...
                for result in results:
                    if isinstance(result, Exception):
                        log.warning(f"BLE write error: {result}")

    async def _apply_idle_single(self, client: BleakClient, cfg: dict):
        """Apply idle state to a single client."""
        idle = cfg.get("idle", DEFAULT_IDLE)
        if not idle.get("on", True):
            cmd = ble_color_cmd(0, 0, 0)
        else:
            r, g, b = apply_brightness(idle["r"], idle["g"], idle["b"], idle.get("brightness", 15))
            cmd = ble_color_cmd(r, g, b)
        try:
            await client.write_gatt_char(BLE_CHAR_UUID, cmd, response=False)
        except Exception as e:
            log.warning(f"Idle apply error: {e}")

    async def apply_idle(self):
        """Apply current idle state to all bulbs."""
        cfg = load_config()
        idle = cfg.get("idle", DEFAULT_IDLE)
        if not idle.get("on", True):
            cmd = ble_color_cmd(0, 0, 0)
        ...
        log.info(f"Idle state applied: {'off' if not idle.get('on') else f'rgb({idle[\"r\"]},{idle[\"g\"]},{idle[\"b\"]}) @ {idle.get(\"brightness\",15)}%'}")

    async def set_color(self, r: int, g: int, b: int, brightness: int):
        """Directly set a color+brightness (from web UI manual control)."""
        if not self._alert_active:
            ar, ag, ab = apply_brightness(r, g, b, brightness)
            await self._write_all(ble_color_cmd(ar, ag, ab))

    async def set_power(self, on: bool):
        """Turn all bulbs on (idle color) or off."""
        if not self._alert_active:
            if on:
    ...
    async def flash(self, r: int, g: int, b: int, brightness: int, pattern_name: str):
        """Start alert flash — cancels any previous flash."""
    ...
    async def _flash_loop(self, r: int, g: int, b: int, pattern_name: str):
        ...
        try:
            ...
            self._alert_active = False
            await self.apply_idle()  # restore idle state when alert ends
    ...
    async def _stop_flash_task(self):
        ...
        if self._flash_task and not self._flash_task.done():
            ...
            try: await self._flash_task
            except asyncio.CancelledError: pass
        self._flash_task = None

    async def stop_alert(self):
        """Stop alert flash and restore idle."""
        await self._stop_flash_task()
        self._alert_active = False
        await self.apply_idle()

    async def test_flash(self):
        """3-second red test flash then restore idle."""
...
# ─── Push notification ────────────────────────────────────────────────────────
def push_notify(cfg: dict, title: str, message: str, priority: str = "high"):
...
def is_relevant(alert: dict, cfg: dict) -> bool:
...
# ─── Main service loop ────────────────────────────────────────────────────────
async def service_loop(ble: BLEManager):
    seen_ids: set = set()
    alert_active = False
    alert_end_time = 0.0
    ...
    while True:
        ...
        for alert in alerts:
            ...
            if cat == "14":
                log.info(f"Early warning | {', '.join(cities)}")
                seen_ids.add(alert_id)
                continue

            if cat == "10":
                log.info(f"All clear | {', '.join(cities)}")
                seen_ids.add(alert_id)
                if alert_active:
            ...
            if not is_relevant(alert, cfg):
                log.info(f"Skipped (other city) | {cat} | {', '.join(cities)}")
                continue

            if cat not in cfg.get("enabled_cats", list(ALERT_DEFAULTS.keys())):
                log.info(f"Skipped (category disabled) | {cat}")
                continue
            ...
            log.info(f"ALERT | {color['name']} | {cities_str} | {desc}")
            ...
            push_notify(
                cfg,
                title=f"🚨 {color['name']}",
                message=f"{cities_str}\n{desc}" if desc else cities_str,
            )
